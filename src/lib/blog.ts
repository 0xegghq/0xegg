...
export function markdownToHtml(md: string): string {
  ...
  function inlineFormat(text: string): string {
    ...
    res = res.replace(/`([^`]+)`/g, '<code class="code-inline">$1</code>');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('|')) {
      ...
      if (!inTable) {
        inTable = true;
        tableHeader = true;
        out.push('<div class="table-wrap"><table class="blog-table">');
      }
      if (trimmed.includes('---')) {
        tableHeader = false;
        continue;
  ...
  if (inCodeBlock) out.push(`<pre><code>${escapeHtml(codeBuffer.join('\n'))}</code></pre>`);
...
export function getAllPosts(): BlogPost[] {
  ...
  let postEntries: Array<{ slug: string; raw: string }> = [];

  // Try Vite import.meta.glob first
  try {
  ...
  // Fallback to fs if glob returned nothing
  ...
  for (const { slug, raw } of postEntries) {
    ...
    posts.push({
      ...
      author: frontmatter.author || '0xEgg Team',
